...
const usePVStore = create(
  persist(
    ...
    { name: 'polevault-tracker-store', storage: createJSONStorage(() => AsyncStorage), version: 1 }
  )
);
...
// Screen wrapper: centered SAU logo behind content
...
function PracticeFormScreen({ navigation }) {
  ...
  const save = () => {
    const sess = { id: shortId(), type: 'practice', date, dayName, goals, steps: steps ? Number(steps) : undefined, takeoffIn: Number(takeoffIn) || 0, standardsIn: Number(standardsIn) || 0, heightIn: Number(heightIn) || 0, notes };
    add(sess);
...
function MeetFormScreen({ navigation }) {
  ...
  const save = () => {
    const sess = { id: shortId(), type: 'meet', date, meetName: meetName?.trim() || undefined, goals, attempts: attempts.map(({ id, ...rest }) => rest), notes };
    add(sess);
...
const styles = StyleSheet.create({
  ...
  bgLogo: {
    ...
    opacity: 0.18, // keep color, just subtle
  },
