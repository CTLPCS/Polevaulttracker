...
// Approach dropdown (ft 1–40, in 1–11), approachIn persisted/displayed, emails updated.
...
const defaultWeeklyPlan = {
  ...
  routine: [
    ...
    '  1) 2× Sidestep hurdles (both ways)',
    '  2) 2× Step over hurdles',
    '  3) 2× Hop over hurdles',
    '  4) 2× Crawl under',
    '  5) 2× Crab crawl',

    'Drills before and during Full Vault Day:',
    '  1) 3×3 Rope drill (w/o platform)',
    '  2) 3×3 Rope drill (w/ platform, partner assist)',
    '  3) 3×5 Ring drill',
    '  4) 10× Bendy pole drill',
    '  5) 3×8 Wall plant w/ comp pole',

    'Runway:',
    '  1) One arm',
    '  2) Sweep',
    '  3) Sweep with turns (¼, ½, full)',
    '  4) Press',
    '  5) Full vault',
  ...
  Tuesday: {
    ...
    routine: [
      'Sprint warm up',
      '2x5 Mini hurdles w/ pole (stay tall; plant after last hurdle and jump)',
      '2x5 Mini hurdles w/o pole (stay tall; jump after last hurdle)',

      'Drills',
      '3x3 rope drill w/o platform (sweep into the drill with drive leg as high as you can)',
      '3x3 rope drill w/ platform (partner pulls you forward; work sweep leg and form the whole drill)',
      '3x5 Ring drill (keep form during the whole swing through)',
      '10x Bendy pole drill (keep chest forward and back hand moving through)',
      '3x5 Medicine ball — sitting',
      '3x5 Medicine ball — standing',
      '3x8 Wall plant w/ comp pole (top hand high/straight; step in and drive knee)',

      'Bubkas — progression to full bubkas',
      'Static bubkas on dip bars (3x10 before moving on)',
      'Negatives on a bar (slow descent)',
      ...
      'Sandbag/weight drag under body (reach under and pull to other side until time)',
      'Rest 30–60s before restarting the circuit; 15–30s between exercises',
    ],
  },
  ...
  Wednesday: {
    ...
    routine: [
      ...
      'Sweep (keep form the whole way; avoid dropping head/shoulders)',
      'Sweep with turns (quarter, half, full)',
      'Press (top hand highest, bottom arm straight, knee driven, sweep leg swings through but not upside down)',
      'Full vault',
    ],
  },
  ...
  Friday: {
    goals: 'Sprint Workout — Ask Kam',
    routine: [
      'Sprint warm up (replace 2 tech runs with mini hurdle runs)',
      '2x5 Mini hurdles w/ pole (stay tall; plant and jump after last)',
      '2x5 Mini hurdles w/o pole (stay tall; jump after last)',

      'Choose one of the sprint sets:',
...
// -------------------- Store (force weeklyPlan from code; don't persist it) --------------------
...
const usePVStore = create(
  persist(
    (set, get) => ({
      ...
      weeklyPlan: defaultWeeklyPlan,   // always sourced from code
      sessions: [],
    ...
    }),
    {
      ...
      // IMPORTANT: bump this above whatever your current version is
      version: 11,

      // Always reset weeklyPlan to code-defined value on load/migrate
      migrate: async (persisted) => {
        // start from whatever was saved (if anything)
        const state = typeof persisted === 'object' && persisted ? { ...persisted } : {};

        // --- force code-defined weekly plan every boot ---
        state.weeklyPlan = defaultWeeklyPlan;

        // make sure other pieces exist + shape is sane
      ...
      // Do NOT save weeklyPlan to storage anymore
      partialize: (state) => ({
        settings: state.settings,
        sessions: state.sessions,
        // weeklyPlan intentionally omitted so edits in code always win
      }),
    }
  )
);
...
// Generic modal dropdown (no deps)
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalCard}>
          ...
          <ScrollView style={{ maxHeight: 360 }}>
            {options.map((opt) => (
              <Pressable key={`${title}-${opt.label}-${opt.value}`} onPress={() => { onSelect(opt); onClose(); }} style={styles.optionRow}>
...
// Screen wrapper with SAU watermark (bottom)
...
// Height input for imperial (ft/in) or metric free cm entry (for takeoff)
function UnitAwareHeightInput({ units, valueInches, onChangeInches, placeholder }) {
  ...
  const [ins, setIns] = useState(fromInches(valueInches).inches.toString());
  ...
  const handleImperial = (f, i) => { setFeet(f); setIns(i); onChangeInches(toInches({ feet: Number(f), inches: Number(i) })); };
  const handleMetric = (c) => { setCm(c); onChangeInches(cmToInches(Number(c))); };

  if (units === 'imperial') {
    return (
      <Row>
        <TextInput value={feet} onChangeText={(t) => handleImperial(t, ins)} keyboardType="number-pad" placeholder="ft" style={styles.inputSmall} />
        <Text style={{ fontSize: 16 }}>ft</Text>
        <TextInput value={ins} onChangeText={(t) => handleImperial(feet, t)} keyboardType="number-pad" placeholder="in" style={styles.inputSmall} />
        <Text style={{ fontSize: 16 }}>in</Text>
      </Row>
    );
  }
  // metric free entry in cm (used for takeoff)
  return (
    <Row>
...
function sessionSummaryText(session, settings, athlete) {
  ...
  if (session.type === 'meet') {
    ...
    const r = session.routine.map((x) => `${x.done ? '[x]' : '[ ]'} ${x.text}`).join('\n');
    middle = `Routine:\n${r}`;
  }

  const notes = session.notes ? `\n\nNotes:\n${session.notes}` : '';
  // Treat any routine string that ends with ":" as a section header
...
function TodayScreen({ navigation }) {
  ...
  // Treat headings (strings ending with ":") as plain labels
  if (r.trim().endsWith(':')) {
    return (
      <Text key={idx} style={{ fontWeight: '700', fontSize: 16, marginVertical: 6 }}>
        {r.replace(/:$/, '')}
      </Text>
    );
  }
  // Otherwise render it as a checkable pill
  return <Pill key={idx} text={r} />;
})}
  ...
  return (
    <Screen>
      ...
      >
        ...
        if (!text) return null;
        if (header) {
          return <Text key={i} style={{ fontWeight: '800', fontSize: 16, marginTop: 6 }}>{text.replace(/:$/, '')}</Text>;
        }
        return <Text key={i} style={styles.pText}>{done ? '✓' : '○'} {text}</Text>;
      })}
    </View>
  </Field>
) : null}
...
function PracticeFormScreen({ navigation }) {
  ...
  const approachFeetOptions = useMemo(
    () => Array.from({ length: 40 }, (_, i) => ({ label: `${i + 1} ft`, value: i + 1 })),
    []
  );
          ...
          {routine.length ? (
            <View style={{ gap: 8 }}>
              {routine.map((item, idx) =>
                item.isHeader ? (
                  <Text key={idx} style={{ fontWeight: '800', fontSize: 16, marginTop: 6 }}>{item.text.replace(/:$/, '')}</Text>
                ) : (
                  <CheckboxChip
                    key={idx}
                    checked={!!item.done}
...
function MeetFormScreen({ navigation }) {
  ...
  const mOptions = useMemo(() => {
    ...
    for (let i = 152; i <= 609.6; i += 1) {
      const m = i / 100;
      const label = `${(m).toFixed(2).replace(/\.?0+$/,'')} m`;
      arr.push({ label, value: metersToInches(m), valueRaw: m });
    }
    return arr;
  }, []);
      ...
      <Section title="New Meet">
        ...
        <Field label="Attempts">
          <View style={{ gap: 10 }}>
            ...
            <ButtonSecondary title={units==='metric' ? 'Add Height (m)' : 'Add Attempt'} onPress={() => {
              if (units === 'metric') setMVisible(true);
              else addAttempt();
            }} />
          </View>
        </Field>
...
// PLAN: read-only view (no editing)
function PlanScreen() {
  const plan = usePVStore((s) => s.weeklyPlan);
...
// SETTINGS: athlete profile inputs + Level dropdown + Units label tweak
    ...
    <Screen>
      ...
      <Section title="Units">
        <Row>
          ...
          <Pressable onPress={() => setUnits('metric')} style={[styles.choice, units === 'metric' && styles.choiceOn]}><Text style={[styles.choiceText, units === 'metric' && styles.choiceTextOn]}>Metric (cm/m)</Text></Pressable>
        </Row>
