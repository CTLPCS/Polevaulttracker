  ...
  persist(
    ...
    }),
    {
      ...
      migrate: async (persisted, fromVersion) => {
        ...
        // normalize session numeric fields
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalCard}>
          ...
          <ScrollView style={{ maxHeight: 360 }}>
            {options.map((opt) => (
              <Pressable key={opt.value} onPress={() => { onSelect(opt); onClose(); }} style={styles.optionRow}>
...
// Height input used for imperial OR metric (cm) free entry (for takeoff)
...
function fmtInchesGeneric(inches, units='imperial'){ // fallback
  const v = Number(inches||0); if(!v) return '—';
  if(units==='metric'){ return `${Math.round(inchesToCm(v))} cm`; }
  const {feet, inches:ins} = fromInches(v); return `${feet}'${ins}"`;
}
                ...
                <Row style={{ justifyContent: 'flex-start', flexWrap: 'wrap' }}>
                  {s.type === 'practice' ? (
                    <>
                      ...
                      <Pill text={`Takeoff ${fmtTakeoff(s.takeoffIn, usePVStore.getState().settings.units)}`} />
                      <Pill text={`Standards ${fmtStandards(s.standardsIn, usePVStore.getState().settings.units)}`} />
                      {s.heightIn ? <Pill text={`Bar ${fmtBar(s.heightIn, usePVStore.getState().settings.units)}`} /> : null}
                    </>
                  ) : (
                    <>
                      {s.attempts?.length ? (<Pill text={`Attempts ${s.attempts.length}`} />) : null}
                      <Pill text={`PR ${fmtBar(calcPR([s]), usePVStore.getState().settings.units) || '—'}`} />
                    </>
                  )}
                </Row>
...
// PRACTICE FORM (routine + metric dropdowns)
function PracticeFormScreen({ navigation }) {
  ...
  const cmOptions = useMemo(() => {
    const arr = [];
    for (let cm = 45; cm <= 80; cm++) {
      arr.push({ label: `${cm} cm`, value: cmToInches(cm), valueRaw: cm });
    }
    return arr;
  }, []);
  const mOptions = useMemo(() => {
    const arr = [];
    // 1.52 → 6.096 in 0.01 m steps
    for (let i = 152; i <= 609.6; i += 1) {
      const m = i / 100; // 0.01 m increments
      const label = `${(m).toFixed(2).replace(/\.?0+$/,'')}${(m%1===0)?'.00':''} m`.replace(/\.00 m$/,' m');
  ...
  const BarChooser = units === 'metric'
    ? (
      <>
        <SimpleDropdown
          label="Select bar height (m)"
          valueLabel={attempts.some(a=>a.heightIn>0) ? null : (0 && 'Select bar height (m)')}
          onPress={() => setMVisible(true)}
        />
        <DropdownModal
          visible={mVisible}
          ...
          options={mOptions}
          onSelect={(opt) => {
            // Add an attempt prefilled with this height
    ...
    : null; // for imperial you'll set attempts with UnitAwareHeightInput per row
