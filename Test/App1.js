...
const fromInches = (total) => {
  ...
  if (inches === 12) { // normalize 5'12" -> 6'0"
...
const usePVStore = create(
  persist(
    (set, get) => ({
      ...
      setDayPlan: (dayName, plan) => set((s) => ({ weeklyPlan: { ...s.weeklyPlan, [dayName]: plan } })),
...
const Pill = ({ text }) => (
  <View style={styles.pill}><Text style={{ fontWeight: '600' }}>{text}</Text></View>
);
...
// Screen wrapper with logo background + SafeArea
const Screen = ({ children, scroll = false, contentContainerStyle }) => {
  return (
    <ImageBackground
      source={require('./assets/sau-logo.png')} // <-- put your SAU logo here
      resizeMode="contain"
      style={styles.bg}
    ...
    >
      <SafeAreaView style={styles.screen}>
        {scroll ? (
          <ScrollView contentContainerStyle={[{ paddingBottom: 40 }, contentContainerStyle]}>
            {children}
          </ScrollView>
        ) : (
          children
...
function UnitAwareHeightInput({ units, valueInches, onChangeInches, placeholder }) {
  // Supports imperial ft/in or metric cm; stores as inches
  ...
  if (units === 'imperial') {
    return (
      <Row>
        <TextInput
          value={feet}
          onChangeText={(t) => handleImperial(t, inches)}
        ...
        <TextInput
          value={inches}
          onChangeText={(t) => handleImperial(feet, t)}
          keyboardType="number-pad"
          placeholder="in"
          style={styles.inputSmall}
        />
        ...
        {sessions.length === 0 ? (
          ...
          <View>
            {sessions.map((s) => (
              <Pressable
                key={s.id}
                onPress={() => navigation.navigate('SessionDetails', { id: s.id })}
                style={styles.card}
              >
                <Row style={{ justifyContent: 'space-between' }}>
                  <Text style={{ fontWeight: '700', fontSize: 16 }}>
                    {s.type === 'meet' ? 'Meet' : 'Practice'} – {new Date(s.date).toLocaleDateString()}
                  </Text>
                  <Text style={styles.muted}>{s.meetName || s.dayName || ''}</Text>
                </Row>
                <View style={{ height: 6 }} />
                <Text style={styles.pText} numberOfLines={2}>
                  Goals: {s.goals || '—'}
                </Text>
                <View style={{ height: 6 }} />
                <Row style={{ justifyContent: 'flex-start', flexWrap: 'wrap' }}>
                  {s.type === 'practice' ? (
                    ...
                    <>
                      {s.attempts?.length ? (
                        <Pill text={`Attempts ${s.attempts.length}`} />
                      ) : null}
...
function SessionDetailsScreen({ route }) {
  ...
  if (!session) return (
    <Screen><Text>Session not found.</Text></Screen>
  );
      ...
      <Section title={`${session.type === 'meet' ? 'Meet' : 'Practice'} – ${new Date(session.date).toLocaleDateString()}`}>
        {session.type === 'meet' ? (
          <>
            ...
            <Field label="PR (best cleared)">
              <Text style={styles.h2}>{fmtInches(calcPR([session]), units) || '—'}</Text>
            </Field>
  ...
  return (
    <Screen scroll>
      <Section title="New Practice">
        ...
        <Field label="Steps (approach)">
          <NumberInput value={steps} onChange={setSteps} placeholder="e.g., 8" />
        </Field>
        <Field label="Takeoff mark">
          <UnitAwareHeightInput
            units={units}
            valueInches={takeoffIn}
            onChangeInches={setTakeoffIn}
            placeholder={units === 'metric' ? 'takeoff cm' : undefined}
          />
        </Field>
        <Field label="Standards setting">
          <UnitAwareHeightInput
            units={units}
            valueInches={standardsIn}
            onChangeInches={setStandardsIn}
            placeholder={units === 'metric' ? 'standards cm' : undefined}
          />
        </Field>
        <Field label="Bar height (optional)">
          <UnitAwareHeightInput
            units={units}
            valueInches={heightIn}
            onChangeInches={setHeightIn}
            placeholder={units === 'metric' ? 'height cm' : undefined}
          />
        </Field>
        <Field label="Notes">
            ...
            {attempts.map((a) => (
              <View key={a.id} style={styles.cardRow}>
                <View style={{ flex: 1 }}>
                  ...
                  <UnitAwareHeightInput
                    units={units}
                    valueInches={a.heightIn}
                    onChangeInches={(v) => updateAttempt(a.id, { heightIn: v })}
                  />
                </View>
                <View style={{ width: 16 }} />
                <View style={{ flex: 1 }}>
                  ...
                  <Row>
                    <Pressable onPress={() => updateAttempt(a.id, { result: 'clear' })} style={[styles.choice, a.result === 'clear' && styles.choiceOn]}>
                      <Text style={[styles.choiceText, a.result === 'clear' && styles.choiceTextOn]}>Clear</Text>
                    </Pressable>
                    <Pressable onPress={() => updateAttempt(a.id, { result: 'miss' })} style={[styles.choice, a.result === 'miss' && styles.choiceOnMiss]}>
                      <Text style={[styles.choiceText, a.result === 'miss' && styles.choiceTextOn]}>Miss</Text>
                    </Pressable>
                    <Pressable onPress={() => removeAttempt(a.id)} style={[styles.choice, { backgroundColor: '#eee' }]}> 
...
function PlanScreen() {
  const plan = usePVStore((s) => s.weeklyPlan);
  const setDayPlan = usePVStore((s) => s.setDayPlan);
  const [activeDay, setActiveDay] = useState(todayName());
  const entry = plan[activeDay] || { goals: '', routine: [] };
  const [goals, setGoals] = useState(entry.goals || '');
  const [routineText, setRoutineText] = useState((entry.routine || []).join('\n'));

  React.useEffect(() => {
    const e = plan[activeDay] || { goals: '', routine: [] };
    setGoals(e.goals || '');
    setRoutineText((e.routine || []).join('\n'));
  }, [activeDay, plan]);

  const save = () => {
    const routine = routineText.split('\n').map((x) => x.trim()).filter(Boolean);
    setDayPlan(activeDay, { goals, routine });
    Alert.alert('Saved', `${activeDay} plan saved.`);
  };
    ...
    <Screen scroll>
      <Section title="Weekly Practice Plan">
        ...
        <Field label={`${activeDay} goals`}>
          <TextInput value={goals} onChangeText={setGoals} placeholder="e.g., Tall at takeoff, hit mid 42'" style={styles.input} />
        </Field>
        <Field label={`${activeDay} routine (one per line)`}>
          <TextInput
            value={routineText}
            onChangeText={setRoutineText}
            placeholder={'Example:\Dynamic warm-up\nPole runs (6x)\n3-step pop-ups'}
            style={[styles.input, { height: 140 }]} multiline
          />
        </Field>
        <ButtonPrimary title="Save Day" onPress={save} />
      </Section>
    </Screen>
  );
}
  ...
  return (
    <Screen scroll>
      <Section title="Highlights">
        <Field label="Personal Record (best cleared)">
          <Text style={styles.h2}>{prInches ? fmtInches(prInches, units) : '—'}</Text>
        </Field>
        <Field label="Average takeoff mark">
          <Text style={styles.h3}>{avgTakeoffIn ? fmtInches(avgTakeoffIn, units) : '—'}</Text>
        </Field>
        <Field label="Average standards">
          <Text style={styles.h3}>{avgStandardsIn ? fmtInches(avgStandardsIn, units) : '—'}</Text>
        </Field>
        <Field label="Average steps">
          <Text style={styles.h3}>{avgSteps ? Number(avgSteps).toFixed(1) : '—'}</Text>
        </Field>
      </Section>
    </Screen>
  );
      ...
      <Section title="Units">
        <Row>
          ...
          <Pressable onPress={() => setUnits('metric')} style={[styles.choice, units === 'metric' && styles.choiceOn]}>
            <Text style={[styles.choiceText, units === 'metric' && styles.choiceTextOn]}>Metric (cm)</Text>
          </Pressable>
        </Row>
      </Section>
      <Section title="About">
        <Text style={styles.pText}>PoleVault Tracker – local-only MVP. Data stays on device.</Text>
      </Section>
    </Screen>
  );
}
...
function avgOf(arr) {
  ...
  const n = arr.length;
  return arr.reduce((a, b) => a + Number(b || 0), 0) / n;
}
  ...
  for (const s of sessions || []) {
    if (s.type === 'meet' && Array.isArray(s.attempts)) {
      for (const a of s.attempts) {
        if (a.result === 'clear') best = Math.max(best, Number(a.heightIn) || 0);
      }
    }
  }
  return best || 0;
...
const styles = StyleSheet.create({
  ...
  // Watermark style: centered, very light
  bgImage: {
    opacity: 0.06,            // tweak for lighter/darker logo
    width: '80%',             // scale of contained image
    height: '80%',
    alignSelf: 'center',
  },
