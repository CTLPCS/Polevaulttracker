  ...
  persist(
    ...
    }),
    {
      ...
      migrate: async (persisted, fromVersion) => {
        // If nothing persisted yet, start fresh
        ...
        // v0/1 → had only settings.units and maybe weeklyPlan/sessions
        if (fromVersion <= 1) {
          const prevUnits = state?.settings?.units ?? 'imperial';
          state.settings = {
            units: prevUnits,
            athlete: { firstName: '', lastName: '', year: '', level: 'highschool' },
          };
        ...
        // v2/3/4/5 → might have athlete but missing fields
        if (fromVersion >= 2 && fromVersion <= 5) {
          const prev = state.settings ?? {};
          const prevAth = prev.athlete ?? {};
          state.settings = {
            units: prev.units ?? 'imperial',
            athlete: {
              firstName: prevAth.firstName ?? '',
              lastName:  prevAth.lastName ?? '',
              year:      prevAth.year ?? '',
              level:     prevAth.level === 'college' ? 'college' : 'highschool',
            },
          };
          // Sessions: ensure arrays and expected keys exist
          if (!Array.isArray(state.sessions)) state.sessions = [];
          state.sessions = state.sessions.map((s) => {
            if (s?.type === 'practice') {
              return {
                ...s,
                steps: Number.isFinite(s.steps) ? s.steps : undefined,
                takeoffIn: Number(s.takeoffIn || 0),
                standardsIn: Number(s.standardsIn || 0),
                heightIn: Number(s.heightIn || 0),
                // routine may not exist previously
                routine: Array.isArray(s.routine) ? s.routine : [],
              };
            }
            if (s?.type === 'meet') {
              return {
        ...
        // Final guard: fill any missing top-level fields
...
// NEW: No-dependency dropdown for Level (Pressable + Alert)
const LevelDropdown = ({ value, onChange }) => {
  const options = [
    { label: 'High School', value: 'highschool' },
    { label: 'College', value: 'college' },
  ];
  const current = options.find(o => o.value === value)?.label || 'Select level';
  ...
  const open = () => {
    Alert.alert(
      ...
      '',
      [
        ...options.map(opt => ({ text: opt.label, onPress: () => onChange(opt.value) })),
  ...
  return (
    <Pressable onPress={open} style={styles.dropdown}>
...
function sessionSummaryText(session, settings, athlete) {
  ...
  const setup = [
    `Steps: ${session.steps ?? '—'}`,
    `Takeoff: ${fmtInches(session.takeoffIn, units)}`,
    `Standards: ${fmtInches(session.standardsIn, units)}`,
    session.heightIn ? `Bar: ${fmtInches(session.heightIn, units)}` : '',
  ].filter(Boolean).join('\n');

  let middle = '';
  if (session.type === 'meet') {
    const attempts = (session.attempts || []).map((a, i) =>
      `  ${i + 1}. ${fmtInches(a.heightIn, units)}  ${a.result === 'clear' ? 'O' : 'X'}`
    );
    const prToday = fmtInches(calcPR([session]), units) || '—';
  ...
  return (
    <Screen>
      <Section title="New Practice">
        ...
        <Field label="Bar height (optional)"><UnitAwareHeightInput units={units} valueInches={heightIn} onChangeInches={setHeightIn} /></Field>
...
// MEET FORM (steps/standards/takeoff + email)
...
// SETTINGS: athlete profile inputs + Level dropdown
      ...
      <Section title="Athlete">
        ...
        <Row style={{ gap: 12 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <LevelDropdown
              value={athlete.level}
              onChange={(val) => setAthleteField('level', val)}
            />
          </View>
        </Row>
      </Section>
...
export default function App() {
  return (
    <NavigationContainer>
      <Stack.Navigator>
        {/* Renamed to avoid name collisions with Tab.Screen "Home" */}
